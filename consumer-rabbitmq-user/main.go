...
func main() {
	...
	go func() {
		for data := range messages {
			...
			//=====================================//
			defer db.Close()
			sqlStatement := `insert into tbusers (iduser,name_user,email,phone,status,username,password) values ($1,$2,$3,$4,$5,$6,$7) returning iduser`

			var id int64

			err2 := db.QueryRow(sqlStatement, data.IDUSER, data.NAME_USER, data.EMAIL, data.PHONE, data.STATUS, data.USERNAME, data.PASSWORD).Scan(&id)
